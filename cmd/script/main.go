...
func main() {
	...
	// Skip automatic cookie scraping on Railway (no display available)
	// Use DEFINED_SESSION_COOKIE env var instead
	if config.DefinedSessionCookie != "" {
		fmt.Println("Using DEFINED_SESSION_COOKIE from environment")
	} else {
		fmt.Println("Warning: DEFINED_SESSION_COOKIE not set, Codex monitor will be skipped")
	}
